...
=== personalEntry
Enthält `TRUE` wenn Eintrag eine natürliche Person beschreibt (einen Leistungsebringer) - d.h. einen HBA Empfänger. Enthält `FALSE` wenn es um einen Eintrag deiner Institution handelt, d.h. SMC-B Empfänger. 
