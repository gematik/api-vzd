...
- KIM Endpunkte werden bei der Synchronisation von LDAP in den FHIR VZD mit dem HealthcareService verlinkt, in welchem Origin="ldap" gesetzt ist. 
- Alle in "komLeData" & "mail"enthaltenen Informationen sind auch in kimData enthalten. Deshalb werden "komLeData" & "mail" nicht in den FHIR VUD synchronisiert.
...
- Aufnahme in den Volltextindex: Endpoint.address, Endpoint.connectionType, Endpoint.payloadType (nur für KIM Adressen oder für alle Endpoints?), Endpoint.status las z.B. "status.active"
