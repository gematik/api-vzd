...
|Endpoint.connectionType  | Abbildung auf https://simplifier.net/vzd-fhir-directory/endpointdirectoryconnectiontype[EndpointConnectionTypeVS]. Für KIM-Adressen wird "kim" als Code eingetragen. 
|Endpoint.connectionType  Extension "kimVersion" | Die KIM Version wird aus dem LDAP Attribut kimData.version entnommen und in Endpoint connectionType Extension "kimVersion" übernommen.
|Endpoint.connectionType  Extension "xxlMailSupported" | Die KIM Option "große Mails" wird aus dem LDAP Attribut kimData.version entnommen ("\+" am Ende des LDAP Attributs) und in connectionType Extension "xxlMailSupported" als "true" übernommen. Falls kein "+" am Ende des LDAP Attributs vorhanden ist, wird xxlMailSupported auf "false" gesetzt.
...
|ou |FAD| Endpoint.extension:endpointFADIdEx | FAD (ID des KIM Anbieters) 
