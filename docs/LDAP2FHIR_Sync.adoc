...
|kimData.appTags |Liste von appTags| Endpoint.payloadType | appTags entsprechend https://simplifier.net/vzd-fhir-directory/endpointdirectorypayloadtype[EndpointDirectoryPayloadType] (muss für KIM erweitert werden). Jedes appTag aus dem LDAP VZD wird in eine Instanz von payloadType synchronisiert.
...
- Für die KIM Version muss ein neues Attribut im Endpoint mit Kardinalität 0..1 definiert werden. Das Attribut muss als Suchparameter nutzbar sein. Für KIM Endpoints ist es verpflichtend, für andere optional. Für die Werte muss ein Code System definiert werden; 1.0, 1.5, 2.0
- Für die Unterstützung von großen KIM-Mails muss ein neues Attribut im Endpoint definiert werden: Entweder ein Boolean Attribut xxl-kim-mails-supported oder ein Wert in connectionType oder payloadType. Das Attribut muss als Suchparameter nutzbar sein.
