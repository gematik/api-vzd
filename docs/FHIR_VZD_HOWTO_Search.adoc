...
|1.1.5 |11.07.25 |FHIR Operations - FHIR VZD endpoint /fdv/search +
FHIR Search Parameters - Overview +
FHIR Search Parameters - Custom Search Parameters
|Added this chapter +
Added some hints +
Added custom Search Parameters
  |gematik

|1.1.6 |23.09.25 | Special Case: Search by ProfessionOIDs in the ePA Context
|Added this chapter 
  |gematik

|1.1.7 |03.02.26 | 4.3 Relevant ProfessionOIDs
|Update fdv-relevant oid-list 
  |gematik

|1.1.8 |06.03.26 | 2.1 FHIRDirectorySearchAPI
|Information about "total" HAPI bug added 
  |gematik
...
IMPORTANT: Note regarding a bug in the HAPI server: In responses to search queries, "total" refers to filtering via Elasticsearch when using full-text search and not to subsequent filtering via the database (non-full-text attributes).
...
- The search with code & CodeSystem may be can be helpful when codes occur in multiple code systems. Example coding of such a search parameter: "&specialty=http://loinc.org|57833-6"
- Using the "_tag" search parameter, you can filter for system resources (_tag=ldap) or owner created resources (_tag=owner).
...
Note: A resourcew may contain several values in the "identifier". This request searches in all identifier values, independent of the identifier coding system.
...
Special characters can be used to specify search queries. For example, if you put the search query in quotation marks, only the results that exactly match the string will be shown. Additional symbols can be used to exclude or combine search terms, for example. Below are the  signs: +
 +
...
The maximum distance is limited to 100 km. +

[NOTE]
--
This operation returns the search result without sorting. If you need a list of organizations sorted by distance, please use operation XXXXX.
--
...
[source]
----
https://fhir-directory-ref.vzd.ti-dienste.de/fdv/search/HealthcareService?organization.active=true&_include=HealthcareService:organization&_include=HealthcareService:location&_text=3-02.2.0000005004.16.738&organization.type=1.2.276.0.76.4.50,1.2.276.0.76.4.51,1.2.276.0.76.4.52,1.2.276.0.76.4.53,1.2.276.0.76.4.54,1.2.276.0.76.4.245,1.2.276.0.76.4.246,1.2.276.0.76.4.247,1.2.276.0.76.4.255,1.2.276.0.76.4.256,1.2.276.0.76.4.257
----
...
== FHIR Operations - FHIR VZD endpoint /fdv/search
...
Instead of a universal approach, tailored search operations that are specifically tailored to different use cases are implemented. +
 +
These optimized interfaces reduce the workload for users and enable simple interaction with the system. +
 +
Hints for all operations

- All search parameters that are not marked "optional" are mandatory parameters and must be specified. If a mandatory parameter is missing, the FHIR VZD responds with error HTTP 400.
- These operations only support the search parameters specified here.

=== Pharmacy radius search (_query=nearPharmacy)
FHIR operation with full-text search for pharmacies sorted by distance.  +
 +
...
- Search is carried out with maximum distance (see "distance" search parameter). If the specified parameter exceeds the configured value, the search radius is limited.
...
- filtering by specialty code(s) when one or more specialty parameters are provided (https://wiki.gematik.de/spaces/DIRSERV/pages/577939788/Operative+betriebliche+Changes+inkl.+Release+Notes#Operative/betrieblicheChangesinkl.ReleaseNotes-VZDFHIR1.2.0-17[VZD FHIR 1.2.0-17 and higher]),
- filtering by characteristics code(s) when one or more characteristics parameters are provided (https://wiki.gematik.de/spaces/DIRSERV/pages/577939788/Operative+betriebliche+Changes+inkl.+Release+Notes#Operative/betrieblicheChangesinkl.ReleaseNotes-VZDFHIR1.2.0-17[VZD FHIR 1.2.0-17 and higher]),
...
|*distance*
|Radius in km. Maximum radius ist limited to 100 km.
|number, int   
...
|*specialty* (optional)
|Code filter for specialty. The parameter can be used multiple times; all specified values are combined with AND logic (https://wiki.gematik.de/spaces/DIRSERV/pages/577939788/Operative+betriebliche+Changes+inkl.+Release+Notes#Operative/betrieblicheChangesinkl.ReleaseNotes-VZDFHIR1.2.0-17[VZD FHIR 1.2.0-17 and higher]).
|String

|*characteristics* (optional)
|Code filter for characteristics. The parameter can be used multiple times; all specified values are combined with AND logic (https://wiki.gematik.de/spaces/DIRSERV/pages/577939788/Operative+betriebliche+Changes+inkl.+Release+Notes#Operative/betrieblicheChangesinkl.ReleaseNotes-VZDFHIR1.2.0-17[VZD FHIR 1.2.0-17 and higher]).
|String
...
Output: Bundle of healthcare services sorted by distance as well as the associated organization and location resources. +
 +
...
=== Organization radius search (_query=near)
[NOTE]
--
This operation is similar to the Pharmacy radius search operation but allows to search for any kind of organization.
--
FHIR operation with full-text search for organizations sorted by distance.  +
 +
Search for all organizations in the specified radius, sorted by distance, filtering for active and visible organizations and optionally
...
|Comma separated list of codes from https://simplifier.net/vzd-fhir-directory/organizationprofessionoid[OrganizationProfessionOID]
|int   
...
=== ePA organization search (_query=epaSearch)

Custom FHIR query for an optimized and performant search for healthcare services in the ePA context. +
+
The query is intended for manufacturers of ePA frontend applications (ePA-FdV) and provides an optimized alternative to complex FHIR search requests containing a large number of ProfessionOIDs. +
+
The search filters for active and visible organizations and

- performs a full-text search if the `text` parameter is provided,
- filters organizations according to the ProfessionOIDs applicable to the specified ePA version,
- optionally includes or excludes DiGA organizations,
- optionally excludes organizations with specified ProfessionOIDs,
- optionally filters for an organization identifier, e.g. a Telematik-ID,
- returns the associated HealthcareService, Organization and Location resources.

The list of allowed ProfessionOIDs is determined by the specified ePA `version`. The corresponding ProfessionOID lists are maintained by the FHIR VZD and do not have to be provided by the client. +
+
This avoids complex search requests containing a large number of ProfessionOIDs combined by OR conditions and therefore reduces the runtime load of the FHIR VZD.

[NOTE]
--
The operation is intended to be used instead of explicitly specifying the complete list of ProfessionOIDs applicable to an ePA version via `organization.type`.

The client specifies the required ePA version. Filtering for the corresponding ProfessionOIDs is performed by the FHIR VZD.
--

==== Input
.Table ePA organization search parameters
|===
|Search parameter |Description |Data typ

|*text*
|Search text, will be compared to the full-text attribute of the FHIR resource.
|String

|*version* (optional)
|ePA version used to determine the applicable list of ProfessionOIDs. Supported values are `3.0` and `3.1`.
|String

|*includeDiga* (optional)
|Controls whether organizations of type DiGA (Digital Health Application) are included in the search result. If set to `false`, DiGA organizations are excluded. The ProfessionOID for DiGA is: 1.2.276.0.76.4.282
|boolean

|*excludeType* (optional)
|ProfessionOID to be explicitly excluded from the search result. The parameter can be used to exclude organization types from the set of ProfessionOIDs applicable to the selected ePA version.
|String

|*identifier* (optional)
|Filters organizations by `Organization.identifier`. This parameter can be used, for example, to search for a specific organization by its Telematik-ID.
|String
...
The operation returns a FHIR Bundle containing the HealthcareService resources matching the search criteria as well as the associated Organization and Location resources. +
+
Only active and visible organizations matching the ProfessionOID configuration of the specified ePA version are returned.

==== Example queries

.Search for HealthcareServices for ePA version 3.1
[source]
----
GET {{fhir_server}}/fdv/search/HealthcareService?_query=epaSearch
    &text=Mustermann
    &version=3.1
----

.Search for HealthcareServices for ePA version 3.1 excluding DiGA
[source]
----
GET {{fhir_server}}/fdv/search/HealthcareService?_query=epaSearch
    &text=Mustermann
...
.Search for HealthcareServices excluding a specific ProfessionOID
[source]
----
GET {{fhir_server}}/fdv/search/HealthcareService?_query=epaSearch
    &text=Mustermann
    &version=3.1
    &excludeType=1.2.276.0.76.4.54
----

.Search for an HealthcareServices by Telematik-ID
[source]
----
GET {{fhir_server}}/fdv/search/HealthcareService?_query=epaSearch
    &version=3.1
    &identifier=1-2arvtst-ap000052
----

==== Restrictions

- Only HealthcareServices are searched. PractitionerRole resources are not part of this operation.
- Only one full-text search value is supported. To search multiple values use whitespaces.
- Combining multiple full-text search values with OR semantics is not supported.
- The applicable ProfessionOIDs are determined by the specified ePA version and do not have to be provided by the client.
- Additional search parameters not documented for this operation are not supported.