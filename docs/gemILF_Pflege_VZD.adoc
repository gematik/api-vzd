...
[width="100%",cols="6%,10%,84%",options="header",]
|===
...
Andere Schreibweisen werden werden nicht zugelassen. 
|V002 |geplant
| Wenn countryCode==DE muss postalCode 5 stellige Nummer sein. 
|V003 |geplant
| countryCode muss 2-Stellige ISO Code sein. 
|V004 |geplant
| Bei SMC-B muss specialization folgenden Format haben: urn:psc:{OID Codesystem}:{Code}, 
...
zu entnehmen sind.
|V005 |geplant
| Bei HBA muss specialization folgenden Format haben: urn:as:{OID Codesystem}:{Code}, wobei CodeSystems und Codes aus https://simplifier.net/vzd-fhir-directory/practitionerqualificationvs[PractitionerQualificationVS] zu entnehmen sind.
|V006 |geplant
| Überschreibt V004 für Apotheken. Bei Apotheken SMC-B müssen für specialization die Codes aus https://simplifier.net/vzd-fhir-directory/pharmacytypecs[PharmacyTypeCS] (nur Code, ohne CodeSystem) verwendet werden.
|V008 |geplant
...
|V012 |geplant
| Für jeden Sektor wird überprüft, welche Attribute obligatorisch sind (Grundlage ist der Implementierungsleitfaden). Wenn Attribute obligatorisch zu setzen sind, dies aber vom Pflegenden nicht befüllt wird, kann der Eintrag nicht angelegt werden. Sektor (aktuell 17) wird anhand TelematikId erkannt.
|V015 |umgesetzt
| Verzeichnisdienst filtert die Leerzeichen (leading, trailing) automatisch aus. Ist das Ausfiltern nicht möglich oder fachlich unzulässig (z.B. im Zertifikat) werden diese Daten mit Fehler abgelehnt. Minimalprüfung: Leerzeichen am Anfang / Ende sind bei allen Attributen unzulässig. 
|V018 |geplant
| Für SMC-B Einträge muss ProfessionOID aus den folgenden Codes entnommen werden: [gemSpec_VZD] Tab_VZD_Mapping_Eintragstyp_und_ProfessionOID (Wertbereich).
|V019 |geplant
| Bei eHBA muss ProfessionOID aus den folgenden Codes entnommen werden: [gemSpec_VZD] Tab_VZD_Mapping_Eintragstyp_und_ProfessionOID (Wertbereich).
