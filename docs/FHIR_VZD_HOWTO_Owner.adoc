...
|1.0.1 |11.12.25 | |New section "General information" |gematik
|===
...
=== General information
==== Encoding of nested lists / Codable Concepts
Values ​​such as HealthcareService.specialty can be delivered to the Owner SST in several ways that are syntactically different but semantically the same. +
 +
Example: +

.A specialty element with multiple coding elements
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.Several specialty elements, each with a coding element
[%collapsible%closed]
====
[source,txt, linenums]
----
...
Both variants are valid according to the code and FHIR profile. However, such inconsistencies could lead to unexpected behavior internally and for third parties if not all variants are considered. This is a general problem and does not apply only to HealthcareService.specialty. +
 +
*Encoding requirement for FHIR VZD clients:* 

- *FHIR VZD clients must encode all values ​​in one child element in Codable Concepts,*
- *FHIR VZD clients must support both encodings when reading FHIR VZD data (nested loop in the query for values ​​of type CodeableConcept)*
