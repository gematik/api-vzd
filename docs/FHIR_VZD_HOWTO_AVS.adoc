...
|1.0.3 |16.12.25 | 2.5 
|Hinweis zum "Bug im VZD FHIR 1.2.0-15" aufgenommen
|gematik

|1.0.4 |09.04.26 | 2.2 
|Hinweis auf Implementierungsbeispiele entfernt, da die Beispiele veraltet sind.
|gematik

|1.0.5 |02.07.26 | 2.5 
|Angabe von Zeiträumen bei Öffnungs- und Schließzeiten erläutert.
|gematik
...
// Verweis auf Beispielimplementierungen auskommentiert, da Beispiele veraltet sind.
//[NOTE]
//====
...
.FHIR-VZD-Daten der Apotheke  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
Die Kontaktdaten werden in den Apps angezeigt und teilweise direkt aufgerufen (z.B. URL, E-Mail-Adresse der Apotheke oder Telefonnummer). +
...
.FHI-Ressource-HealthcareService - aktuelle Daten aus dem FHIR-VZD  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
*Besonderheiten bei der Eintragung von Sonderöffnungs-, Schließ- und Sonderschließzeiten*

Bei der Eintragung von Sonderöffnungs-, Schließ- und Sonderschließzeiten sollten Sie folgende Punkte beachten:

- Anfangszeit muss kleiner Endzeit sein - das folgende Beispiel wäre ungültig:
=====
[source,txt, linenums]
-----
{
  "description": "Ferien",
  "during": {
    "start": "2025-10-22T00:00:00+02:00",
    "end": "2025-10-21T00:00:00+02:00"
}
-----
=====
- gleiche Anfangs- und Endzeit mit Zeitangabe führt zur Öffnung/Schließung von nur 1 Sekunde
=====
[source,txt, linenums]
-----
{
  "description": "Betriebsschulung",
  "during": {
    "start": "2025-10-22T00:00:00+02:00",
    "end": "2025-10-22T00:00:00+02:00"
}
-----
=====
- stattdessen sollte ein ganzer Öffnungs-/Schließtag folgendermaßen abgebildet werden:
=====
[source,txt, linenums]
-----
{
...
.*Bug im VZD FHIR 1.2.0-15*
CAUTION: Mit VZD FHIR 1.2.0-15 wurden die HealthcareService.specialty-Container für Apotheken auf den Wert „apo-vzd“ konsolidiert.
Derzeit wird „apo-vzd“ jedoch bei der Anlieferung über die Owner-Schnittstelle fälschlicherweise verworfen.
In der Folge führt eine Aktualisierung (PUT) mit dem Wert „apo-vzd“ dazu, dass die hinterlegten Apotheken-Spezialisierungen geleert werden. +
 +
Workaround: Vor der Aktualisierung (PUT) des HealthcareService muss der "text" „apo-vzd“ unterhalb des Attributs "Specialty" entfernt werden (auch wenn "text" „apo-vzd“ bereits im Datensatz steht). 
Diese Vorgehensweise ist bis zur Fehlerbehebung auch im VZD SSP entsprechend umgesetzt. +
 +
Fix im kommenden Release FHIR VZD 1.2.0-16: Der "text" „apo-vzd“ wird bei der Anlieferung korrekt berücksichtigt und die übermittelten Specialty-Werte werden übernommen.

.Der aktualisierte HealthcareService (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.Body von dem POST  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.Ergebnis von dem POST  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.Body von dem PUT  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.FHI-Ressource-Organization - aktuelle Daten aus dem FHIR-VZD  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
...
.FHI-Ressource Location - aktuelle Daten aus dem FHIR-VZD  (Klicken zum Ausklappen)
[%collapsible%closed]
====
[source,txt, linenums]
----
